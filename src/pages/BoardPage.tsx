    ...
    >
      ...
      {isYourTurn ? (
        <>
          ...
          />
          <p className="mb-1 text-xs font-medium text-slate-400">Or pick manually — dimmed cards aren't legal right now:</p>
          <HandStrip cards={you.hand} legalCardIds={legalIds} onPlay={(cardId) => handlePlay(you.id, cardId)} />
        </>
      ) : (
        <Card className="mb-4">
          <p className="mb-2 text-sm font-medium text-slate-300">What did {currentPlayer?.name} play?</p>
